...
\\
\noindent
\textbf{Layer-sequential Unit-variance (LSUV)} (\nurl{https://arxiv.org/abs/1511.06422}{Mishkin and Matas 2015}). Tested out on feed-forward networks with results comparable to BN, greedy pre-trainig, and other initialization schemes, LSUV offers a cheap initialization procedure which effectively does a single round of BN at initialization time. There's not a lot of background on LSUV use, nor its extensions to RNNs.
\\
\noindent
...
\\
\noindent
\textbf{Curriculum learning} (\nurl{https://arxiv.org/abs/1410.4615}{Zaremba and Sutskever 2014}). A stochastic curriculum of training examples generally ordered from easy to hard but with some mixing has proven essentiall in some hard-to-learn RNNs, as compared to minibatch SGD over the entire dataset).
\\
\noindent
