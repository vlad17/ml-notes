...
in 12.9 TODO link along with other model selection approaches besides
subset selection.
...
MSE to var reduction and options
...
Superior ME by variance reduction as seen above

Can be seen explicitly with a resample-style ME expression.

Orhtonormal spcial case.

Selection of regression for general case via CV with eq 12.53
as explicit full form.

GCV replacement. Golub 1979 for discussion, see pros and cons.

**** Garrote and Lasso

Refer to tibs, tibs book, and buhlmann book. This ref is outdated
(interestingly it paints it as a compromise between subset selection
and l2. similarly, modern sources look at it as a convex relaxation of
l0, but have established much stronger sets of guarantees)

Include tibs in this repo (separate folder?)

Just describe lasso objective.

*** 12.6 Bayesian Methods

TODO read through and notes

*** 12.7 

link to Will optimal IaS paper

Provide simple example of IaS problem

Link to optimal Jason lee paper for lasso IaS

Philosophically, what are the different goals for inference if we're
considering differnet models?

what else? TODO

*** 12.8 

Computationally, stepwise methods can rely on incremental add columns methods
TODO link section

GE allows fas backtracking but is unstable. Use QR instead (notes from
chapter here).

APR interestingly doesn't require the full 2^k. Use pruning techniques.
Describe regression tree method.

*** 12.9 Comparison

**** Asymptotic Regimes

Finite-sample too hard, rely on asymptotics.

Which are equivalent? (CV, aic, mallows)
vs BIC vs Shrinkage vs Bayesian?

Who wins when?

lasso for p>n under assumptions.

**** PE estimation

If goal is just selection, we don't care that a PE estimate for an
adaptively selected model is biased, but sometimes we want to get the PE
for interpretibility.

Discussed CV, GCV before. CV can be nested easily and with K-fold somewhat
efficient.

But also little bootstrap is an option.

** Open Questions

agnostic PE and infer guarantees relative to KL optimizer.

Note this isn’t just ml pac where K/dims is fixed and n-> inf.
Need ols structure for diverting params.
Lots of recent regulation work for lasso shows oracle properties
But what about POSI? Best kl model

Equivalent inference questions interesting too
